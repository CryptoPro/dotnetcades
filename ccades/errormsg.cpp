...
{
    CAtlStringW messageW = GetErrorMessage(HRESULT_FROM_WIN32(err), 0x409);
    wchar_t buff[14];
    swprintf(buff, 14, L" (0x%08X)", err);
    buff[13] = 0;
    messageW.Append(buff);
    CAtlStringA messageA = CAtlStringA(CW2A(messageW, CP_UTF8));
    char* szMessage = (char*)calloc(messageA.GetLength() + 1, sizeof(char));
    strcpy(szMessage, messageA.GetBuffer());
    return szMessage;
}